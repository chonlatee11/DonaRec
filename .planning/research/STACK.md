...
**Domain:** Web-based donation e-receipt system for a Thai hospital (back-office RBAC + server-side PDF + transactional email + PDPA-grade PII handling + gap-less fiscal-year receipt numbering)
**Researched:** 2026-06-22
**Confidence:** HIGH on the load-bearing decisions (PostgreSQL, gap-less counter pattern, Thai-PDF via headless Chromium). MEDIUM on framework choice (NestJS is recommended but Spring Boot / Django are defensible alternatives — see rationale).
...
**Native PostgreSQL sequences will NOT satisfy FR-16.** `nextval()` is deliberately non-transactional: a value fetched then rolled back is *never reused*, leaving gaps. This is by design and unavoidable with sequences. (PostgreSQL docs explicitly say: "In the rare case that you really need a gap-less series of values, a sequence is not the right solution.")

**Correct pattern — a counter table locked per fiscal year:**
...
At approval time, inside the **same transaction** that flips the donation to "issued" and writes the receipt row:

1. `INSERT INTO receipt_counter(fiscal_year, last_value) VALUES ($fy, 0) ON CONFLICT DO NOTHING;`
2. `SELECT last_value + 1 FROM receipt_counter WHERE fiscal_year = $fy FOR UPDATE;` — the `FOR UPDATE` row lock serializes all concurrent approvers for that year; the second transaction blocks until the first commits.
3. `UPDATE receipt_counter SET last_value = $n WHERE fiscal_year = $fy;`
4. Insert receipt with that number; **commit.** The number is only "spent" if the transaction commits, so a rollback/rejection leaves no gap.

Fiscal year is derived from the **approval (issue) date** per FR-18 (Oct–Sep cycle, Oct–Dec rolls to next FY). Cancellations use a `cancelled` status, never delete the number (FR-19).

**Implication for the ORM choice:** Prisma has **no native `SELECT … FOR UPDATE`** (open issue #17136). You must run step 2 as a single `$queryRaw` statement inside a Prisma **interactive transaction** (`prisma.$transaction(async (tx) => { … })`). This works and is documented, but it is the one place you drop out of the ORM abstraction.

> If the team prefers pessimistic locking as a first-class ORM feature, **TypeORM** (query-builder `.setLock('pessimistic_write')`) or **Drizzle** are alternatives. This is the single strongest argument against Prisma for this project. Recommendation stands with Prisma + raw counter because the locked region is tiny and well-contained, and Prisma's type safety pays off across the other ~30 requirements.

**Keep the locking transaction short** — the row lock serializes approvals for that fiscal year. Do PDF rendering and email sending *after* commit (via the BullMQ queue), not inside the locked transaction.

Confidence: **HIGH** (PostgreSQL official docs + multiple independent corroborations).

### 2. Thai-script PDF with watermark + embedded signature

**The trap:** Pure-JS PDF builders (`pdf-lib`, `jsPDF`, `pdfkit`) can *embed* a Thai font but have **documented shaping bugs** — stacked Thai vowels and tone marks misalign (pdf-lib issue #675). A Thai tax receipt with garbled diacritics is unacceptable for a legal document.

**The reliable approach:** render an HTML/CSS template through a **real browser engine** (Playwright/Chromium `page.pdf()`). The browser's text-shaping engine handles Thai complex-script layout correctly out of the box. This also makes the rest trivial:
...
**Critical container requirement:** install the Thai font in the image — `apt-get install fonts-thai-tlwg` *and/or* bake TH Sarabun New `.ttf` and reference it via `@font-face`. Without it Chromium renders tofu boxes (□□□). Use `waitUntil: 'networkidle'` before `page.pdf()` so fonts/images settle. Consider `--font-render-hinting=none` for cleaner kerning.

**Operational note:** keep a warm browser pool, ≥1GB RAM for the render service, health-check/restart logic. Run rendering via the job queue to hit NFR-07 (~2–3s/receipt).

Confidence: **HIGH** that headless-Chromium HTML→PDF renders Thai correctly; **HIGH** that the pure-JS libs have shaping issues.

> **Java alternative if the org is JVM-shop:** iText 7 **+ the pdfCalligraph add-on** renders Thai correctly with automatic complex-script shaping and has native watermark APIs — but pdfCalligraph is a **commercial-licensed** module. **Python alternative:** WeasyPrint (HTML/CSS → PDF) also renders Thai well via its text engine. Both are valid if the framework choice changes.

---

## PII Encryption-at-Rest (PDPA — NFR-02, FR-29)

National ID / tax ID is the sensitive field. PDPA also requires limiting *who* can see it by role (section 7 of the requirements).

**Recommended: application-level AES-256-GCM with envelope encryption.**
...
`pgcrypto` (`pgp_sym_encrypt(…, 'cipher-algo=aes256')`) is acceptable for *less* sensitive confidential fields where DB-admin exposure is tolerable, but **not** the primary protection for national IDs.

Confidence: **HIGH** (aligns with PDPA guidance and PostgreSQL encryption docs).

---

## Email Delivery (FR-25/26/27/28)

**Requirements:** PDF attachment, 2-language templates, delivery status (success/fail) tracking + resend, staff fallback download.

**Recommended: Amazon SES** (region `ap-southeast-1` / Singapore for data-residency proximity to Thailand) via the AWS SDK, with SNS-based bounce/delivery notifications, OR **Postmark** if you want managed deliverability + richer webhooks with less plumbing.
...
Confidence: **MEDIUM-HIGH** (provider landscape is stable; exact provider depends on hospital procurement/data-residency policy — flag for stakeholder confirmation).
...
npm install @nestjs/core @nestjs/common @nestjs/platform-express \
  @nestjs/passport passport passport-jwt @nestjs/jwt argon2 \
  @casl/ability @nestjs/config zod
...
npm install @aws-sdk/client-ses        # or: npm install postmark
npm install exceljs file-type sharp multer
npm install nestjs-i18n i18next

# Dev
npm install -D typescript vitest @types/node prisma eslint prettier
```
...
**If the hospital IT is a Java/.NET shop (procurement constraint):**
...
**If PKI digital signing is added later (Open Issue #1):**
...
**If/when the public donation form (Phase 2, Flow B) launches:**
...
---
*Stack research for: Thai hospital donation e-receipt system (DonaRec)*
*Researched: 2026-06-22*
