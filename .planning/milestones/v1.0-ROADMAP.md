...
- [x] 04-02-PLAN.md — Chrome sidecar (fonts-thai-tlwg) + chromedp v0.14.2 pin + testcontainers helper
- [x] 04-04-PLAN.md — EmailSender interface + dev capture + bilingual receipt/email i18n (TDD)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 04-03-PLAN.md — PDF render core (TDD): sandboxed Thai/EN Chromium render + golden-file + JS/network security regression

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-05-PLAN.md — Outbox worker pipeline (TDD): poll→render→freeze PDF→email→record + retry/backoff dead-letter
...
- [x] 04-07-PLAN.md — Admin settings config store + template/image validation + HTML/real-PDF preview + E2E (TDD)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 04-08-PLAN.md — Admin Settings UI (Screen 6): four tabs + sandboxed live preview + real-PDF + save-all
...
- [x] 06-01-PLAN.md — Backend data foundation: source column (000015) + public-web system-user seed (000016, fixed UUID) + source filter on Search/CountDonations
- [x] 06-02-PLAN.md — Backend anti-automation primitives (TDD): Turnstile verifier (fail-closed) + per-IP rate-limit middleware
- [x] 06-05-PLAN.md — Frontend (app)/(public) route-group split + scoped warm dual-theme + fonts + Turnstile dep
...
- [x] 06-07-PLAN.md — Staff pending-review queue (Screen 11) + SourceBadge + source filter + Screen 1/3 amendments (FR-08)
...
### Phase 7: Close gap: Flow B composite E2E (public-submit through approve to issued receipt)
