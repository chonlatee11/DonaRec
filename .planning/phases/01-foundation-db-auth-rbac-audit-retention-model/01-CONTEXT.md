...
- **D-03 [informational]:** **⚠️ DEVIATION จาก ROADMAP SC#1** — Success Criterion ข้อ 1 เขียน "assign exactly one of the Maker/Checker/Admin roles" แต่ผู้ใช้เลือก multi-role → SC#1 ต้องอัปเดต roadmap หลังเฟสนี้ (ดู Deferred → roadmap edit)
...
- **D-09 [deferred]:** ไม่มี MFA/OTP ใน MVP (Keycloak เปิดเพิ่มได้ภายหลังโดยไม่แก้แอป)
...
- **D-22 [deferred]:** Frontend = **React / Next.js** (back-office UI; Phase 6 public form ใช้ตัวเดียวกัน) — ไม่ build ใน Phase 1 (backend foundation only); เริ่ม UI เฟสถัดไป
